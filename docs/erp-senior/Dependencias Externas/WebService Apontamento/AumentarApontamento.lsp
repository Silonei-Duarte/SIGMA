...
AumentarApontamento.tabelaEntradas.linhaatual = 0;
vaDados = AumentarApontamento.tabelaEntradas.valor;
...
Se (wacao = "INCREMENTAR-OP")
  {
    ...
    aLogPrc = "Inserido por INCREMENTAR-OP";
    ...
    Se ((nQtdRe1 > 0) e (nJaApt = 0))
      {
        ...
        @ Filtra fora os componentes de quantidade fixa; aqui o calculo e sempre proporcional. @
        TClaSql = "Select a.CodCmp WWCodCmp, \
                  ...
                  Where a.CodEmp = :nCodEmp \
                    ...
                    And (m.TipQtd <> 'F' OR m.TipQtd IS NULL) \
                    And a.USU_BxaOrp = 'S' \
        ...
        Enquanto (SQL_EOF(CCurCmp) = 0)
          {
             ...
             @ Proporcionaliza pela quantidade apontada na OP. @
             Se (nPrvOop <> 0)
               {
...
Se (nTipoRetorno = 0)
  {
    ...
    AumentarApontamento.waRetorno = vaRetorno;
  }
